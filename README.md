...
Repositório dedicado aos manifests Kubernetes do **frontend** da solução Digital Step Flow. Contém apenas a definição dos recursos para deploy do frontend em clusters Kubernetes (Kustomize base + overlays dev/prod).

## Objetivo

Este repositório é atualizado automaticamente pelo pipeline de CI/CD do repositório [pucrs-tcc-digital-step-flow-frontend](https://github.com/raphaelmoraes/pucrs-tcc-digital-step-flow-frontend): após build e push da nova imagem, o job **Deploy to Dev** (no `ci.yml`) ou o workflow **CD Deploy PROD** atualiza a tag da imagem em `k8s/dev/kustomization.yaml` ou `k8s/prod/kustomization.yaml` e faz push para este repositório. O Argo CD (ou aplicação de deploy) sincroniza o cluster com as alterações.
...
O Deployment utiliza a imagem **raphaelmoraes/digital-step-flow-frontend** (construída a partir da imagem base hardened; ver documentação no repositório do frontend). A tag é definida nos overlays:

- **dev:** tag de desenvolvimento (ex.: `24.13.0-r1-dev`), atualizada pelo job Deploy to Dev no `ci.yml` do frontend.
- **prod:** tag de produção (ex.: `0.0.1`), atualizada pelo workflow CD Deploy PROD do frontend.
...
- Ajuste no `argocd/application.yaml` o `source.repoURL` para a URL deste repositório e o `source.path` para `k8s/dev` ou `k8s/prod` conforme o ambiente.
...
- **pucrs-tcc-digital-step-flow-frontend:** código da aplicação, Dockerfile, CI/CD. O pipeline desse repositório atualiza a tag da imagem neste repositório (frontend-k8s) nos arquivos `k8s/dev/kustomization.yaml` e `k8s/prod/kustomization.yaml`.
